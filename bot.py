...
def check_subscription(user_id):
    try:
        for channel in CHANNELS:
            ...
            status = bot.get_chat_member(chat_info.id, user_id).status
...
def start(message):
    ...
    if not check_subscription(user_id):
        ...
        for channel in CHANNELS:
            markup.add(types.InlineKeyboardButton("اشترك الآن", url=channel))
        bot.send_message(user_id, "مرحباً بك عزيزي!

للاستمرار في استخدام البوت، الرجاء الاشتراك في القنوات التالية:", reply_markup=markup)
        return
    ...
    bot.send_message(user_id, f"مرحباً {first_name}!
